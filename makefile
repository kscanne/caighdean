...
rules.txt-refresh: $(GRAMADOIR)/morph-ga.txt
	rm -f rules.txt
	cat $(GRAMADOIR)/morph-ga.txt | iconv -f iso-8859-1 -t utf8 | egrep -v '^#' | sed '/^\^h?an-(\[bcfgmp\]h/,/eachtar.freas/s/^/#/' | sed '/^(\.\[aouáóú/,/^(\[eiéí.*-ne/s/^/#/' | sed '/íní?/s/^/#/' | sed '/^\^do(/,/^\^h?in(\[\^/s/^/#/' | sed 's/^\([^ \t]*\)[ \t]*\([^ \t]*\)[ \t]*\([^ \t]*\).*/\1\t\2\t\3/' > rules.txt
	chmod 444 rules.txt
